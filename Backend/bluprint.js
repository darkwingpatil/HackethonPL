  ...
  socket.on("userQuries", async (input) => {
    let res = await openai.createChatCompletion({
      ...
      messages: [
        {
          role: "user",
          content: `First check this query ${input.message} is related to ${topic} or not, if it is not related then don't reply with the details for the user query. Otherwise provide some suggestions to that query within 100 words`,
        },
      ],
    });
  ...
  for (let i = 0; i < MyLab.length - 1; i++) {
    ...
    let res = await openai.createChatCompletion({
      ...
      messages: [
        {
          role: "user",
          content: `First check this query ${input} is related to ${topic} or not, if it is not related then don't reply with the details for the user query. Otherwise provide some suggestions to that query within 100 words`,
        },
      ],
    });
